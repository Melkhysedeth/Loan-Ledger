...
export default function LoanDetail() {
    ...
    const classification = !isInactive
        ...
        : 'ok'
    const isOverdue = !isInactive && (classification === 'overdue' || loan.status === 'overdue')
    ...
    const STATUS = {
        ...
        cancelled: { label: 'Cancelado', color: 'text-red-300 bg-red-900/20', dot: 'bg-red-300' },
    }
    const status = STATUS[loan.status] || STATUS.active

    const proximaFecha = paymentsMade === 0
        ? (loan.first_payment_date ? new Date(loan.first_payment_date + 'T12:00:00') : null)
        : nextPayment
        ...
        <div className="pb-32 bg-gray-50 dark:bg-gray-950 min-h-screen">
            ...
            <div className="px-4 pt-6 pb-4 flex items-center gap-3">
                ...
                <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-400">
                        Otorgado el {loan.start_date ? new Date(loan.start_date + 'T12:00:00').toLocaleDateString('es-CO', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'}
                    </p>
                </div>
            </div>
            ...
            {isFrozen && (
                <div className="mx-4 mb-3 bg-blue-900/20 border border-blue-700/40 rounded-2xl px-4 py-3 flex gap-3 items-start">
                    ...
                    <div>
                        <p className="text-sm font-semibold text-blue-400">Préstamo congelado</p>
                        <p className="text-xs text-blue-300/70 mt-0.5">
                            Sin intereses · Capital pendiente: <span className="font-bold">{formatCOP(remaining)}</span>
                        </p>
                        <p className="text-xs text-blue-300/50 mt-0.5">Al descongelar se pactarán nuevas condiciones</p>
                    </div>
                </div>
            )}
            ...
            {isCancelled && (
                <div className="mx-4 mb-3 bg-red-900/20 border border-red-700/40 rounded-2xl px-4 py-3 flex gap-3 items-start">
                    ...
                    <div>
                        <p className="text-sm font-semibold text-red-400">Préstamo cancelado</p>
                        {loan.cancellation_note && (
                            <p className="text-xs text-red-300/70 mt-0.5">{loan.cancellation_note}</p>
                        )}
                    </div>
                </div>
            )}
            ...
            <div className="mx-4 bg-white dark:bg-gray-800 rounded-2xl p-4 mb-3 shadow-sm">
                    ...
                    <div className="text-right shrink-0">
                        {isFrozen && (
                            <>
                                ...
                                <p className="text-xs font-semibold text-blue-400">{formatCOP(remaining)}</p>
                            </>
                        )}
                        {isPaid && (
                        ...
                        {!isInactive && nextPayment && (
                            <>
                                <p className="text-[10px] text-gray-400">{isOverdue ? 'Vencido desde' : 'Próximo pago'}</p>
                                <p className={`text-xs font-semibold flex items-center gap-1 justify-end mt-0.5 ${isOverdue ? 'text-red-500' : 'text-blue-500'}`}>
                                    <Calendar size={10} />
                                    {proximaFecha?.toLocaleDateString('es-CO', { day: 'numeric', month: 'short', year: 'numeric' })}
                                </p>
                            </>
                        )}
                    </div>
                ...
                <div className="grid grid-cols-4 gap-2 pt-3 border-t border-gray-100 dark:border-gray-700">
                    ...
                    <div>
                        ...
                        <p className={`text-xs font-bold ${isPaid || isCancelled ? 'text-gray-400' : 'text-amber-500'}`}>{formatCOP(remaining)}</p>
                    </div>
                    <div>
            ...
            <div className="mx-4 bg-white dark:bg-gray-800 rounded-2xl p-4 mb-3 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <DetailCell Icon={Calendar} label="Fecha inicio" value={loan.start_date ? new Date(loan.start_date + 'T12:00:00').toLocaleDateString('es-CO', { day: 'numeric', month: 'short', year: 'numeric' }) : '—'} />
                    ...
                    <DetailCell
                        ...
                        valueColor={isOverdue ? 'text-red-500' : 'text-gray-800 dark:text-gray-100'}
                    />
                    <DetailCell
                        Icon={CreditCard}
            ...
            {!isPaid && !isCancelled && (
                <>
                    <div className="mx-4 grid grid-cols-3 gap-2 mb-2">
                        {!isFrozen && (
                            <ActionBtn Icon={DollarSign} label="Registrar pago" color="bg-blue-600" onClick={() => navigate(`/loans/${id}/pay`)} />
                        )}
                    ...
                    <div className="mx-4 grid grid-cols-3 gap-2 mb-2">
                        ...
                        />
                        <ActionBtn Icon={CheckCircle} label="Liquidar" color="bg-green-700/80" onClick={() => setModal('settle')} />
                        <ActionBtn Icon={Ban} label="Cancelar" color="bg-red-700/80" onClick={() => setModal('cancel')} />
                    </div>
                </>
            )}
        ...
        <ModalWrapper title="Modificar interés" onClose={onClose}>
            <div className="space-y-3">
                <div className="bg-gray-800 rounded-xl px-3 py-2 text-xs text-gray-400">
                    Tipo actual: <span className={`font-semibold ${isVariable ? 'text-purple-400' : 'text-blue-400'}`}>
                        {isVariable ? 'Variable (sobre saldo)' : 'Fijo (sobre monto original)'}
                    </span>
                </div>
                <div>
                ...
                {parsedRate > 0 && (
                    <p className="text-sm text-gray-400">
                        {isVariable
                            ? <>Próximo interés sobre saldo <span className="text-white">({formatCOP(remaining)})</span>: <span className="text-white font-semibold">{formatCOP(previewInterest)}</span></>
                            : <>Interés fijo por cuota: <span className="text-white font-semibold">{formatCOP(previewInterest)}</span></>
                        }
                    </p>
                )}
                <button
...
function FreezeModal({ loan, remaining, onClose, onDone }) {
    ...
    const [interestType, setInterestType] = useState(loan.interest_type || 'fixed')

    const parsedRate = parseFloat(rate) || 0

    // FIX: fijo → sobre loan.amount | variable → sobre remaining
    const previewInterest = interestType === 'variable'
        ? Math.round((remaining * parsedRate) / 100)
        : Math.round((loan.amount * parsedRate) / 100)
            ...
            <ModalWrapper title="Descongelar préstamo" onClose={onClose}>
                <div className="space-y-3">
                    <div className="bg-blue-900/20 border border-blue-700/30 rounded-xl p-3 text-xs text-blue-300">
                        Capital pendiente: <span className="font-bold text-white">{formatCOP(remaining)}</span> · Pacta las nuevas condiciones para reactivar.
                    </div>

                    <div>
                        <label className="text-sm text-gray-400 mb-1 block">Nueva fecha de primer pago *</label>
                        <input
                            type="date"
                    ...
                    <div>
                        <label className="text-sm text-gray-400 mb-2 block">Tipo de interés</label>
                        <div className="grid grid-cols-2 gap-2">
                            <button
                            ...
                            >
                                Fijo
                                <span className="block text-[10px] font-normal opacity-70 mt-0.5">Sobre monto original</span>
                            </button>
                            <button
                            ...
                            >
                                Variable
                                <span className="block text-[10px] font-normal opacity-70 mt-0.5">Sobre saldo pendiente</span>
                            </button>
                        </div>
                    </div>

                    <div>
                        <label className="text-sm text-gray-400 mb-1 block">Tasa de interés (%)</label>
                        <input
                            type="number"
                            value={rate}
                            onChange={e => setRate(e.target.value)}
                        ...
                        {parsedRate > 0 && (
                            <p className="text-xs text-gray-400 mt-1">
                                Interés por cuota: <span className="text-white font-semibold">{formatCOP(previewInterest)}</span>
                                <span className="ml-1 opacity-60">
                                    ({interestType === 'variable' ? `${parsedRate}% sobre ${formatCOP(remaining)}` : `${parsedRate}% sobre ${formatCOP(loan.amount)}`})
                                </span>
                            </p>
                        )}
                    </div>
    ...
    return (
        <ModalWrapper title="Congelar préstamo" onClose={onClose}>
            <p className="text-gray-400 text-sm mb-4">
                El préstamo quedará congelado. No se generarán intereses ni se mostrará mora hasta descongelarlo.
            </p>
            <button
            ...
            <div className="space-y-3">
                {canSettle ? (
                    ...
                    <div className="bg-gray-800 border border-gray-700 rounded-xl p-3 text-xs text-gray-400">
                        El capital pendiente es <span className="font-bold text-white">{formatCOP(remaining)}</span>. No hay saldo por liquidar.
                    </div>
                )}
                <p className="text-gray-400 text-sm">
                    Esta acción marcará el préstamo como <span className="text-green-400 font-semibold">Liquidado</span> y no se podrá revertir fácilmente.
                </p>
                <button
                    onClick={handleSave}
        ...
        <ModalWrapper title="Acuerdo especial" onClose={onClose}>
            <div className="space-y-3">
                <div className="bg-amber-900/20 border border-amber-700/30 rounded-xl p-3 text-xs text-amber-300">
                    El préstamo pasará a <strong>sin intereses</strong>. Solo se registrarán abonos a capital.
                </div>
                <div>
        ...
        <ModalWrapper title="Cancelar préstamo" onClose={onClose}>
            <div className="space-y-3">
                <div className="bg-red-900/20 border border-red-700/30 rounded-xl p-3 text-xs text-red-300">
                    El capital pendiente de <span className="font-bold text-white">{formatCOP(remaining)}</span> volverá al saldo disponible del negocio al cancelar.
                    {remaining < loan.amount && (
                        <p className="mt-1 text-red-300/60">
                            Los pagos ya realizados ({formatCOP(loan.amount - remaining)}) quedan en el historial.
                        </p>
                    )}
                </div>
                <div>
                    <label className="text-sm text-gray-400 mb-1 block">Motivo de cancelación *</label>
                    <textarea
                        rows={3}
                        value={note}
                        onChange={e => setNote(e.target.value)}
                    ...
                    />
                    {!note.trim() && <p className="text-xs text-red-400 mt-1">El motivo es obligatorio</p>}
                </div>
                <button
                    onClick={handleSave}
